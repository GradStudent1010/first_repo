...
Cov($e_i$,$e_j$) = E[($Y_i$ - $\bar{Y}$)($Y_j$-$\bar{Y}$)] - E[($Y_i$ - $\bar{Y}$)]E[($Y_j$ - $\bar{Y}$)]
...
tree <- read.csv(file = 'Tree_Equity_Scores_Tucson_noNA.csv')


nrow(tree)

attach(tree)
plot(PCTTreeCover ~ PCTPoverty)

fitted <- lm(PCTTreeCover ~ PCTPoverty, data = tree)

ggplot(tree, aes(x=PCTPoverty, y= PCTTreeCover)) + geom_point() + stat_smooth(method='lm')

summary(fitted)

newdata = data.frame(PCTPoverty = 50 )
predict(fitted, newdata)

MLE <- sum(resid(fitted)^2)/nrow(tree)
print (MLE)
